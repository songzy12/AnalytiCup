...
if __name__ == '__main__':
    ...
    q2_data = pad_sequences(question2_word_sequences,
                            maxlen=MAX_SEQUENCE_LENGTH)
    test1_data = pad_sequences(
        test1_word_sequences, maxlen=MAX_SEQUENCE_LENGTH)
    test2_data = pad_sequences(
        test2_word_sequences, maxlen=MAX_SEQUENCE_LENGTH)
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=TEST_SPLIT, random_state=RNG_SEED)
    Q1_train = X_train[:, 0]
    Q2_train = X_train[:, 1]
    Q1_test = X_test[:, 0]
    Q2_test = X_test[:, 1]
    ...
    callbacks = [ModelCheckpoint(
        MODEL_WEIGHTS_FILE, monitor='val_loss', save_best_only=True)]
    ...
    max_val_acc, idx = max((val, idx)
                           for (idx, val) in enumerate(history.history['val_loss']))
    print('Maximum validation accuracy = {0:.4f} (epoch {1:d})'.format(
        max_val_acc, idx + 1))

    # Evaluate the model with best validation accuracy on the test partition
    model.load_weights(MODEL_WEIGHTS_FILE)
    loss, accuracy = model.evaluate([Q1_test, Q2_test], y_test, verbose=0)
    print('Test loss = {0:.4f}, test accuracy = {1:.4f}'.format(
        loss, accuracy))
    result = model.predict([test1_data, test2_data])

    sub = pd.DataFrame(result)
    sub.to_csv('../output/quora-submission.txt', index=False,
