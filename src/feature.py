...
def get_feature_vec(df, tokenizer):
    ...
    df['word2vec_es0'] = [sum([embedding_matrix[x] for x in data]) / len(
        data) if data else np.random.normal(emb_mean, emb_std, (embed_size)) for data in df['seq_es0']]
    df['word2vec_es1'] = [sum([embedding_matrix[x] for x in data]) / len(
        data) if data else np.random.normal(emb_mean, emb_std, (embed_size)) for data in df['seq_es1']]
...
def get_feature_jaccard(df):
    ...
    df['jaccard'] = df.apply(lambda row: jaccard(
        row['seq_es0'], row['seq_es1']), axis=1)
    return df
